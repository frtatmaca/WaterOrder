var maillist_index = (function ($) {
    ...
    function activity(MessageId, SessionId, SenderUserId, SenderUserName, Subject, FileName, SessionName, CreatedDate) {
        this.MessageId = ko.observable(MessageId);
        this.SessionId = ko.observable(SessionId);
        this.SenderUserId = ko.observable(SenderUserId);
    ...
    function getAdvancedSearchFilters(skip, top) {
        ...
        return { "searchKey": searchKey, "skip": skip, "top": top };
    }
        ...
        for (var i = 0; i < results().length; i++) {
            item.ActivityList.push(
                new activity(
                    results()[i].MessageId(),
                    results()[i].SessionId(),
                    results()[i].SenderUserId(),
                    results()[i].SenderUserName() != null ? results()[i].SenderUserName() : "",
                    results()[i].Subject(),
                    results()[i].FileName(),
                    results()[i].SessionName(),
