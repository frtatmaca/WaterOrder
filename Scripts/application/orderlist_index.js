var orderlist_index = (function ($) {
    ...
    pub.statusReplace = function (el) {
        ...
        $.ajax({
            url: "/Home/StatusReplace/",
            type: "POST",
            dataType: "json",
            data: { orderId: actId, status: actStatus },
            success: function (res) {
                if (res == "success") {
                    if (actStatus == 1) {
                        $(".tr_" + actId).css("background-color", "blue");
                    } else
                    {
                        $(".tr_" + actId).css("background-color", "green");
                    }
                    console.log("Statü değiştirildi");
                } else {
                    console.log("Statü değiştirilir iken bir hata ile karşılaşıldı.");
                }
            },
            error: function (res) {
                console.log("Error in process.");
    ...
    function buildPagerDiv() {
        ...
        var html = '<li class="first paginate_button paginate_button_disabled"><a href="javascript:orderlist_index.pagerAction(1, 0);">' + "First" + '</a></li>';
        $("#ul_paging").append(html);

        html = '<li class="prev paginate_button paginate_button_disabled"><a href="javascript:orderlist_index.pagerAction(2, 0);">← ' + "Prev" + '</a></li>';
        $("#ul_paging").append(html);

        if (item.totalPageNumber() < 5) {
            for (var i = 1; i <= item.totalPageNumber() ; i++) {
                html = '<li id="nmbr' + i + '" data-id="' + i + '"><a href="javascript:orderlist_index.pagerAction(5, ' + i + ');">' + i + '</a></li>';
                $("#ul_paging").append(html);
            }
        }
        else {
        ...
        html = '<li class="next"><a href="javascript:orderlist_index.pagerAction(3, 0);">' + "Next" + ' →</a></li>';
        $("#ul_paging").append(html);

        html = '<li class="last"><a href="javascript:orderlist_index.pagerAction(4, 0);">' + "Last" + '</a></li>';
        $("#ul_paging").append(html);
    }
